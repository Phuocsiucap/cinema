...
// Mở rộng interface CastMember để xử lý file upload ở client
...
const MediaInput = ({
  ...
  // Nếu có value (URL từ DB) nhưng chưa có preview (file mới), mode mặc định là URL
  // Nếu người dùng upload file, mode sẽ tự chuyển sang upload hoặc người dùng tự click
            ...
            <div className="border-2 border-dashed border-gray-600 rounded-lg p-6 hover:bg-gray-800 transition-colors text-center cursor-pointer relative group">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    onFileChange(file);
                    onChange(''); // Clear URL string if file is selected
                  }
                }}
              />
...
// --- COMPONENT: CAST PHOTO INPUT (Nhỏ gọn) ---
...
export function EditMoviePage() {
  ...
  // Cast members state with file support
  ...
  // File states for Poster & Background
  const [posterFile, setPosterFile] = useState<File | null>(null);
  const [posterPreview, setPosterPreview] = useState<string | null>(null);
  const [bgFile, setBgFile] = useState<File | null>(null);
  const [bgPreview, setBgPreview] = useState<string | null>(null);

  // Original URLs for comparison (to avoid re-uploading unchanged images)
  const [originalUrls, setOriginalUrls] = useState<{
    poster_url: string;
    background_url: string;
    cast_photos: Record<string, string>; // actor_id -> photo_url
  }>({ poster_url: '', background_url: '', cast_photos: {} });
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      ...
      try {
        const movie: Movie = await movieService.getMovie(id);
        ...
        // Save original URLs for comparison
        const castData = movie.actor_associations || movie.cast || [];
        const castPhotos: Record<string, string> = {};
        castData.forEach(c => {
          if (c.actor.id) castPhotos[c.actor.id] = c.actor.photo_url || '';
        });
        setOriginalUrls({
          poster_url: movie.poster_url || '',
          background_url: movie.background_url || '',
          cast_photos: castPhotos,
        });
        ...
        // Parse cast members & initialize file states (reusing castData from above)
        if (castData.length > 0) {
          const members: CastMemberState[] = castData.map(c => ({
            actor_id: c.actor.id,
            name: c.actor.name,
            photo_url: c.actor.photo_url || '',
  ...
  const handleAddCastMember = () => {
    setCastMembers((prev) => [...prev, { name: '', role_name: '', photo_url: '', photo_file: null, photo_preview: null }]);
  };
  ...
  const handleCastMemberChange = (index: number, field: keyof CastMemberState, value: any) => {
    setCastMembers((prev) => prev.map((member, i) => i === index ? { ...member, [field]: value } : member));
  };

  const handleCastPhotoFileChange = (index: number, file: File) => {
    const reader = new FileReader();
    reader.onloadend = () => {
      setCastMembers((prev) => prev.map((member, i) => i === index ? {
        ...member,
        photo_file: file,
  ...
  const searchActorsFromAPI = useCallback(async (query: string, index: number) => {
    if (!query.trim()) { setActorSuggestions([]); setShowSuggestions(false); return; }
    try {
      ...
      if (index === actorSearchIndex) { setActorSuggestions(suggestions); setShowSuggestions(suggestions.length > 0); }
  ...
  const handleSelectActor = (index: number, actor: ActorSuggestion) => {
    setCastMembers((prev) => prev.map((member, i) => i === index ? {
      ...member, actor_id: actor.id, name: actor.name, photo_url: actor.photo_url, photo_file: null, photo_preview: null
    } : member));
    setShowSuggestions(false); setActorSuggestions([]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else if (formData.poster_url && formData.poster_url.trim() && formData.poster_url !== originalUrls.poster_url) {
        // Upload URL to Cloudinary only if URL has changed
      ...
      } else if (formData.background_url && formData.background_url.trim() && formData.background_url !== originalUrls.background_url) {
        // Upload URL to Cloudinary only if URL has changed
        ...
        castMembers
          ...
          .map(async ({ photo_file, photo_preview, ...rest }) => {
            if (photo_file) {
              ...
              const originalPhotoUrl = rest.actor_id ? originalUrls.cast_photos[rest.actor_id] : '';
              if (rest.photo_url !== originalPhotoUrl) {
                // Upload URL to Cloudinary only if changed
    ...
    <AdminLayout>
      <div className="p-8">
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          ...
          <ChevronRight size={16} />
          <span className="text-white">Edit Movie</span>
        </div>
        <h1 className="text-3xl font-bold text-white mb-8">Edit Movie</h1>

        <div className="flex gap-6 border-b border-gray-800 mb-8">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`pb-4 px-2 text-sm font-medium transition-colors relative ${activeTab === tab.id ? 'text-red-500' : 'text-gray-400 hover:text-white'}`}>
              {tab.label} {activeTab === tab.id && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-red-500" />}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'basic' && (
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea name="description" value={formData.description} onChange={handleInputChange} rows={3} className="w-full px-4 py-3 bg-[#2a2a2a] border border-gray-700 rounded-lg text-white resize-none focus:outline-none focus:ring-2 focus:ring-red-500" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Duration (min) <span className="text-red-500">*</span></label>
                      ...
                      <div className="flex items-start gap-4">
                        <div className="w-20 h-20 bg-gray-800 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 border border-gray-600">
                          {(member.photo_preview || member.photo_url) ? (
                            <img src={member.photo_preview || member.photo_url} alt={member.name} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/80?text=Error'; }} />
                        ...
                        <div className="flex-1 grid grid-cols-3 gap-4">
                          <div className="relative" ref={actorSearchIndex === index ? suggestionRef : null}>
                            ...
                            <div className="relative">
                              <input type="text" value={member.name} onChange={(e) => handleActorNameChange(index, e.target.value)} onFocus={() => { setActorSearchIndex(index); if (member.name?.trim()) searchActorsFromAPI(member.name, index); }} placeholder="Search actor..." className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500 pr-8" />
                          ...
                          <div>
                            ...
                            <input type="text" value={member.role_name || ''} onChange={(e) => handleCastMemberChange(index, 'role_name', e.target.value)} placeholder="Character name" className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
                          </div>
                          <div>
                            <CastPhotoInput
                              value={member.photo_url || ''}
                              onChange={(val) => { handleCastMemberChange(index, 'photo_url', val); handleCastMemberChange(index, 'photo_preview', null); handleCastMemberChange(index, 'photo_file', null); }}
